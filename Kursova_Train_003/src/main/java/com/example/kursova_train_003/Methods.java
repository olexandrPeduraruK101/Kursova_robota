...
public class Methods {
    ...
    //заброньоване місце після додавання білетв
    public void addTicketAndBookSeat(int trainId, int idSeatsBuy) {
        try {
            con = DBConnextion.getCon();

            // Оновлення значення поля "booked_seats" у таблиці "поїзд"
            String updateTrainQuery = "UPDATE train_database_001.train SET booked_seats = booked_seats + 1 WHERE id = ?";
            st = con.prepareStatement(updateTrainQuery);
            st.setInt(1, trainId);
            st.executeUpdate();

            // Оновлення значення поля "booked" у таблиці "seats_buy"
            String updateSeatsQuery = "UPDATE train_database_001.seats_buy SET booked = true WHERE train_id = ? AND id = ?";
            st = con.prepareStatement(updateSeatsQuery);
            st.setInt(1, trainId);
            st.setInt(2, idSeatsBuy);
            st.executeUpdate();
    ...
    //звільнення місця після видаляння
    public void removeTicketAndFreeSeat(Tickets tickets) {
        try {
            con = DBConnextion.getCon();
            ...
            String query = "SELECT train_id, seat_id FROM train_database_001.tickets WHERE id = ?";
            st = con.prepareStatement(query);
            st.setInt(1, tickets.getId());
            rs = st.executeQuery();

            if (rs.next()) {
                tickets.setTrainId(rs.getInt("train_id"));
                tickets.setSeatId(rs.getInt("seat_id"));
            ...
            // Оновлення значення поля "booked" у таблиці "seats_buy"
            String updateSeatsQuery = "UPDATE train_database_001.seats_buy SET booked = false WHERE train_id = ? AND id = ?";
            st = con.prepareStatement(updateSeatsQuery);
            st.setInt(1, tickets.getTrainId());
            st.setInt(2, tickets.getSeatId());
            st.executeUpdate();
